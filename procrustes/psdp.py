...
def psdp_projgrad(
    ...
    options = {
        ...
        # terminates
        "s_tol": 1e-5,
        # The minimum ratio of ||F_i - F_{i - 1}||_F with 1 + ||F_{i - 1}||_F, below which
    ...
    max_eig = np.max(x) ** 2  # Not sure if these can be complex too, if they are complex, then
    # need to take norm
    BAT = B@A.conj().T
    ...
    while i <= options["max_iter"]:
        ...
        # If the ||S_i - S_{i - 1}||_F / ||S_1 - S_0||_F is lesser than some tolerance level, then
        # we terminate the algorithm. TODO: to decide if this is useful or not.
        if i == 1:
